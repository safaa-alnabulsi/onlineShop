...
$this->menu = array(
    ...
    array(
        ...
        'url' => array('/user'),
        'visible' => Yii::app()->user->checkAccess('User.ProfileField.Admin')
    ),
    array(
        'label' => UserModule::t('Edit'),
        'url' => array('edit'),
        'visible' => Yii::app()->user->checkAccess('User.Profile.Edit')
    ),
    array(
        'label' => UserModule::t('Change password'),
        'url' => array('changepassword'),
        'visible' => Yii::app()->user->checkAccess('User.Profile.Changepassword')
    ),
    array(
        ...
        if ($profileFields) {
            foreach ($profileFields as $field) {
                ...
                <tr>
                    ...
                    <td><?php echo(($field->widgetView($profile)) ? $field->widgetView($profile) : CHtml::encode((($field->range) ? Profile::range($field->range,
                            $profile->getAttribute($field->varname)) : $profile->getAttribute($field->varname)))); ?></td>
                </tr>
            <?php
